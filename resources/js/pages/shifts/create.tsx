...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Neu', href: shiftsRoute.create().url },
];
...
export default function ShiftCreate({ sites, users, defaults }: Props) {
    const { data, setData, post, processing, errors } = useForm({
        site_id: defaults?.site_id ?? sites[0]?.id ?? '',
    ...
    const submit = (event: FormEvent) => {
        ...
        post(shiftsRoute.store().url, { data });
    };
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Neue Schicht" />
            <form onSubmit={submit} className="flex flex-col gap-6 rounded-xl p-4">
                <div>
                    <h1 className="text-xl font-semibold">Neue Schicht</h1>
                    <p className="text-sm text-muted-foreground">Schicht anlegen und Objekt zuweisen.</p>
                </div>
