...
export default function AbsenceEdit({ absence, users, canAssignUser }: Props) {
    const normalized = 'data' in absence ? absence.data : absence;
    const { auth } = usePage().props as { auth: { user: { id: number } } };

    const { data, setData, put, processing, errors } = useForm({
        user_id: normalized.user_id ?? auth.user.id,
    ...
    const submit = (event: FormEvent) => {
        ...
        put(absencesRoute.update(normalized.id).url);
    };

    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Abwesenheiten', href: absencesRoute.index().url },
        { title: normalized.user?.name ?? 'Abwesenheit', href: absencesRoute.edit(normalized.id).url },
    ];

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Abwesenheit bearbeiten" />
            <form onSubmit={submit} className="flex flex-col gap-6 rounded-xl p-4">
                <div>
                    <h1 className="text-xl font-semibold">Abwesenheit bearbeiten</h1>
                    <p className="text-sm text-muted-foreground">Daten der Abwesenheit aktualisieren.</p>
                </div>
                ...
                <div className="flex justify-end gap-2">
                    <button
                        type="button"
                        onClick={() => router.delete(absencesRoute.destroy(normalized.id).url)}
